...
class BookControllerTest {
  ...
  void getById() throws Exception {
    ...
    mockMvc
        .perform(get("/api/books/{id}", sampleId))
        .andExpect(status().isOk())
  ...
  void getById_notFound() throws Exception {
    ...
    mockMvc.perform(get("/api/books/{id}", unknownId)).andExpect(status().isNotFound());
  ...
  void getByTitle() throws Exception {
    ...
    mockMvc
        .perform(get("/api/books/search/title").param("title", "Petit"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.length()").value(1))
        .andExpect(jsonPath("$[0].title").value("Le Petit Prince"));
  ...
  void getByGender() throws Exception {
    ...
    mockMvc
        .perform(get("/api/books/search/gender/{genderId}", genreId))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.length()").value(1))
        .andExpect(jsonPath("$[0].title").value("Le Petit Prince"));
  ...
  void getByDate() throws Exception {
    ...
    mockMvc
        .perform(get("/api/books/search/date").param("date", "1943-04-06"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.length()").value(1))
        .andExpect(jsonPath("$[0].title").value("Le Petit Prince"));
  ...
  void delete() throws Exception {
    ...
    mockMvc
        .perform(MockMvcRequestBuilders.delete("/api/books/{id}", sampleId))
